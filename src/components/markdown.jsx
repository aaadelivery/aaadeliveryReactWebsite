...
function markDownBold(str) {
  ...
  const replaceWith = boldList.map(
    (x) =>
      "<bold className='bold-inline'>" + x[1].replace(/\*\*>/g, "") + "</bold>"
  );
...
function markdownlinks(mdContents) {
  ...
  if (matches) {
    ...
    for (var i = 0; i < matches.length; i++) {
      ...
      res = res.replace(
        matches[i],
        `<a className='red-link' href='${text[2]}'>${text[1]}</a>`
      );
    }
    return res;
  } else {
...
function markdownToHtml(mdContents) {
  ...
  var el = md
    ...
    .replace(
      /<a href\b(?![^>]*\bhttp)([^>]*)>(.*?)<\/a>/gi,
      '<Link className="red-link" to$1>$2</Link>'
    )
        ...
        >
          ...
          >
            {index % 2 == 1 && (
              <>
                <Grid
                  ...
                  xs={12}
                  md={6}
                  style={{ marginTop: "5%", padding: "0 5vw" }}
                >
            ...
            <Grid justifyContent="center" item xs={12} md={6}>
              {(element.pgimg.thumbnail == "/animated-svg-circle-map" ||
                element.pgimg.thumbnail == null) && (
                <div style={{ width: "80%" }}>
